...
static int ft5x0x_read_Touchdata(void)
{
	...
	for (i = 0; i < event->touch_point; i++)
	{
		...
		printk("event->au16_x[%d]=%d\n",i,event->au16_x[i]);
		printk("event->au16_y[%d]=%d\n",i,event->au16_y[i]);
...
static void ft5x0x_report_value(void)
{
	...
	for (i  = 0; i < event->touch_point; i++)
	{
		...
		if (event->au16_x[i] < TSP_MAX_X && event->au16_y[i] < TSP_MAX_Y)
		{
			...
			input_report_abs(data->input_dev, ABS_MT_WIDTH_MAJOR, 10);
			input_report_abs(data->input_dev, ABS_MT_TRACKING_ID, event->au8_finger_id[i]);
			if (event->au8_touch_event[i]== 0 || event->au8_touch_event[i] == 2)
			{
				input_report_abs(data->input_dev, ABS_MT_TOUCH_MAJOR, 200);//event->pressure);
			}
			else
			{
