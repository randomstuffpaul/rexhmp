...
static int get_freq_index(int cluster, unsigned int target_freq)
{
	int i, index = -EINVAL;
	struct cpufreq_frequency_table *table = exynos_info[cluster]->freq_table;

	if (target_freq == CPUFREQ_ENTRY_INVALID)
		return index;

	for (i = 0; (table[i].frequency != CPUFREQ_TABLE_END); i++) {
		if (target_freq == table[i].frequency) {
			index = i;
			break;
		}
	}
				       ...
				       unsigned long pm_event, void *v)
{
	...
	case PM_SUSPEND_PREPARE:
		...
		cur_index = get_freq_index(boot_cluster,
						freqs[boot_cluster]->old);
		exynos_info[boot_cluster]->set_freq(cur_index,
				exynos_info[boot_cluster]->boot_freq_idx);
		freqs[boot_cluster]->old = exynos_info[boot_cluster]->boot_freq;
