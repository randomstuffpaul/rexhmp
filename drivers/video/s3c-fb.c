...
static int s3c_fb_blank(int blank_mode, struct fb_info *info)
{
	...
	case FB_BLANK_NORMAL:
		...
		exynos4_update_media_layers(TYPE_FIMD1, 0);
#else
...
#elif defined(CONFIG_FIMD_USE_WIN_OVERLAP_CNT)
#if defined(CONFIG_ARCH_EXYNOS4)
		exynos4_update_media_layers(TYPE_FIMD1, 1);
#else
...
#if defined(CONFIG_ARCH_EXYNOS4)
		exynos4_update_media_layers(TYPE_FIMD1, regs->win_overlap_cnt);
#else
...
#if defined(CONFIG_ARCH_EXYNOS4)
		exynos4_update_media_layers(TYPE_RESOLUTION, RESOLUTION_FULLHD);
#else
		exynos5_update_media_layers(TYPE_RESOLUTION, RESOLUTION_FULLHD);
#endif
	} else if (pd->win[default_win]->virtual_x == 2560) {
#if defined(CONFIG_ARCH_EXYNOS4)
		exynos4_update_media_layers(TYPE_RESOLUTION, RESOLUTION_WQXGA);
#else
